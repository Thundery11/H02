...
// blogsRouter.get('/', 
// async (req: RequestWithQueryAndBody<{desc: string, searchNameTerm: string | null, sortBy: string, sortDirection: string, pageNumber: number, pageSize: number, createdAt: string}>, res: Response)=>{
...
//     const allBlogs: blogsDbType[] = await blogsCollection.find({}).sort({[sortBy]: sortDirection === 'desc' ? -1 : 1}).skip(skip).limit(pageSize).toArray()
